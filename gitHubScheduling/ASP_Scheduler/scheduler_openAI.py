...
client = OpenAI(api_key="sk-fab31ae7a91746e89a346a57fc3a7a5c", base_url="https://api.deepseek.com")


# Get instance 
def get_instance(problem_description, instance_description):
...
def get_generator(problem_description, instance_template):
...
Given a target goal, you will make an ASP generator that will generate candidate solutions for the following problem:
{problem_description}
...
def get_hard_constraints(hard_constraint_descriptions, problem_description, instance_template, generator, printer=False):
    ...
    for constraint_description in hard_constraint_descriptions:
        ...
        if 'type: count' in constraint_description.lower():
            hard_constraint =  get_count_hard_constraints(constraint_description, problem_description, instance_template, generator)
        elif 'type: sum' in constraint_description.lower():
            hard_constraint =  get_sum_hard_constraints(constraint_description, problem_description, instance_template, generator)
        else:
            hard_constraint =  get_regular_hard_constraints(constraint_description, problem_description, instance_template, generator)
...
def get_regular_hard_constraints(prompt, problem_description, instance_template, generator):
...
def get_count_hard_constraints(prompt, problem_description, instance_template, generator):
...
def get_sum_hard_constraints(prompt, problem_description, instance_template, generator):
    # Remove the last line (the one that says "type: count")
    lines = prompt.splitlines()
    prompt = "\n".join(lines[:-1])
...
def get_soft_constraints(soft_constraint_descriptions, problem_description, instance_template, generator, printer=False):
    ...
    for constraint_description in soft_constraint_descriptions:
        ...
        if 'type: count' in constraint_description.lower():
            soft_constraint =  get_count_soft_constraint(constraint_description, problem_description, instance_template, generator)
        elif 'type: sum' in constraint_description.lower():
            soft_constraint =  get_sum_soft_constraint(constraint_description, problem_description, instance_template, generator)
        else:
            soft_constraint =  get_regular_soft_constraint(constraint_description, problem_description, instance_template, generator)
...
# 
def get_regular_soft_constraint(prompt, problem_description, instance_template, generator):
    system_prompt = f'''You are a bot that is tasked with turning textual hard constraint data into a set of Answer Set Programming (ASP) facts.
Given a set of constraints, you will turn them into ASP facts.
...
def get_count_soft_constraint(prompt, problem_description, instance_template, generator):
    system_prompt = f'''You are a bot that is tasked with turning textual hard constraint data into a set of Answer Set Programming (ASP) facts.
Given a set of constraints, you will turn them into ASP facts.
...
def get_sum_soft_constraint(prompt, problem_description, instance_template, generator):
    system_prompt = f'''You are a bot that is tasked with turning textual hard constraint data into a set of Answer Set Programming (ASP) facts.
Given a set of constraints, you will turn them into ASP facts.
...
def full_ASP_program(problem, printer=False):
    ...
    instance_template = get_instance(problem_description,instance_description)
    ...
    generator = get_generator(generator_description, instance_template)
    print('\n\nGenerator\n' + generator) if printer else None

    # Use instance and generator to create hard constraints
    hard_constraints = get_hard_constraints(hard_constraint_descriptions, problem_description, instance_template, generator, printer=printer)
    
    # Use instance and generator to create soft constraints
    soft_constraints = get_soft_constraints(soft_constraint_descriptions, problem_description, instance_template, generator, printer=printer)
